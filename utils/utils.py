...
def minmax_norm(arr):
    if arr.max() == arr.min():
        return arr  # 如果最大值等于最小值，直接返回原数组，避免除零
    return (arr - arr.min()) / (arr.max() - arr.min())
...
def store_ligand_score(ligand_smiles, atom_types, atom_scores, ligand_path):
    ...
    for i, atom in enumerate(mol.GetAtoms()):
        ...
        if atom_types[i] == atom.GetSymbol():
            atom.SetProp("Atom_Score", str(atom_scores[i]))
        else:
            return False
        ...
        if save_interpret:
            for pair_id in df[matching_row]['ID']:
                ...
                protein_interpret = pd.DataFrame({
                    ...
                    'Residue_Score':minmax_norm(unbatched_residue_score[idx].cpu().flatten().numpy())
                    })

                protein_interpret['Residue_ID'] = protein_interpret.index + 1
                protein_interpret['Residue_Percentile'] = percentile_rank(protein_interpret['Residue_Score'])
                protein_interpret = protein_interpret[['Residue_ID','Residue_Type','Residue_Score','Residue_Percentile']]
...
def virtual_screeningp(screen_df, model, data_loader, result_path, save_interpret=True, ligand_dict=None, device='cpu',
                        save_cluster=False):
    if "ID" in screen_df.columns:
        # Iterate through the DataFrame check any empty pairs 
        for i, row in screen_df.iterrows():
    ...
    # === 在这里展开 mcls_preds，生成三列 ===
    if mcls_preds:
        mcls_preds = np.concatenate(mcls_preds, axis=0)  # 拼成 [N,3]
        screen_df['predicted_antagonist'] = mcls_preds[:,0]
        screen_df['predicted_nonbinder'] = mcls_preds[:,1]
        screen_df['predicted_agonist'] = mcls_preds[:,2]
