...
class net(torch.nn.Module):
    ...
    def forward(self,
        ...
        for idx in range(self.total_layer):
            ...
            # --- NEW: DenseGCN-style non-linear update on pooled cluster features (residual) ---
            # Implements: h_pgl+1 = h_rs_l + DenseGCN(h_rs_l, A_rs)
            # Minimal implementation: apply a small non-linear transform (linear + ReLU) and add as residual.
            # We reuse existing residue_lins[idx] as the linear transform to avoid adding new layers.
            cluster_x = cluster_x + F.relu(self.residue_lins[idx](cluster_x))
            # -------------------------------------------------------------------------
            ...
            # --- NEW: Map atom features back to clique/subgraph features (h_cli update) ---
            # Implements: h_cli = h_cli_feat + Lin(scatter(h_mgl+1))
            # atom2clique_index = (row, col) where `row` is atom index tensor, `col` is corresponding clique index tensor
            row, col = atom2clique_index
            if clique_x is not None and atom_x is not None:
                # aggregate atom features for each clique (mean)
                # dim_size ensures correct size even when some cliques have no atoms in a batch (safety)
                clique_agg = scatter(atom_x[row], col, dim=0, dim_size=clique_x.size(0), reduce='mean')
                # linear + nonlinearity and residual add to clique features
                clique_x = clique_x + F.relu(self.c2a_mlps[idx](clique_agg))
            # -------------------------------------------------------------------------
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, eps, amsgrad):
        ...
        decay = set()
        no_decay = set()  
        whitelist_weight_modules = (torch.nn.Linear, torch_geometric.nn.dense.linear.Linear,torch.nn.Conv2d)
        blacklist_weight_modules = (torch.nn.LayerNorm, torch.nn.Embedding, GraphNorm, PosLinear,torch.nn.BatchNorm2d)
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                fpn = '%s.%s' % (mn, pn) if mn else pn  # full param name
                ...
                #     # weights of blacklist modules will NOT be weight decayed
                #     no_decay.add(fpn)
                # elif pn.endswith('spline_weight') or pn.endswith('base_weight'):
                #     decay.add(fpn)
                # elif pn.endswith('spline_scaler'):
                #     no_decay.add(fpn)
                # elif 'self_attn.in_proj_weight' in fpn or 'norm' in fpn:
                #     no_decay.add(fpn)
                # elif 'atom_type_encoder.weight' in fpn or 'atom_feat_encoder.FC_layers' in fpn:
                #     decay.add(fpn)
                # elif 'atom_feat_encoder.out_ln.weight' in fpn:
                #     no_decay.add(fpn)
                # elif 'clique_encoder.weight' in fpn:
                #     decay.add(fpn)
                # elif 'prot_evo.FC_layers' in fpn:
                #     decay.add(fpn)   
                # else:
                #     raise ValueError(f"Unhandled parameter name: {fpn}")
