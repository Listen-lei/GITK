...
class Drug_PNAConv(nn.Module):
    ...
    def forward(self, atom_x, bond_x, atom_edge_index):
        ...
        batch = Batch(x=atom_x, edge_index=atom_edge_index, edge_attr=bond_x)
        batch = self.gps_layer(batch)
        atom_x1= atom_in + F.relu(batch.x)
...
class Protein_PNAConv(nn.Module):
    ...
    def forward(self, x, prot_edge_index, prot_edge_attr):
        ...
        batch = Batch(x=x_in, edge_index=prot_edge_index, edge_attr=prot_edge_attr)
        batch = self.gps_layer(batch)
        new_x1= x_in + F.relu(batch.x)
