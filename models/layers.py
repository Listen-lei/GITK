...
from models.KAGNN import KAN_linear
# from graphgps.layer.gps_layer import GPSLayer
from grit.layer.grit_layer import GritTransformerLayer
from grit.network.grit_model import GritTransformer
...
class Drug_PNAConv(nn.Module):
    def __init__(self, mol_deg, hidden_channels, edge_channels,
        ...
        # self.gps_layer = GPSLayer(dim_h=hidden_channels,
        #                     local_gnn_type='GAT',
        #                     num_heads=5,
        #                     pna_degrees=mol_deg,
        ...
        # self.gps_layer = GritTransformerLayer(in_dim=hidden_channels,out_dim=hidden_channels,num_heads=5,cfg=cfg)
        self.gps_layer = GritTransformer(dim_in=hidden_channels, dim_out=hidden_channels)
    ...
    def forward(self, atom_x, bond_x, atom_edge_index):
        ...
        deg = pyg.utils.degree(atom_edge_index[1], num_nodes=atom_in.size(0))

        # 计算对数度数
        log_deg = torch.log(deg + 1).unsqueeze(-1)

        # 创建 Batch 对象，并将度数和对数度数添加到属性中
               
        # print(f"x size: {atom_in.size()}")
        # print(f"atom_edge_index size: {atom_edge_index.size()}")
        # print(f"edge_attr size: {bond_x.size()}")
        # print(f"deg size: {deg.size()}")
        # print(f"log_deg size: {log_deg.size()}")

        batch = Batch(x=atom_in, edge_index=atom_edge_index, edge_attr=bond_x, deg=deg, log_deg=log_deg)
        
        assert batch.edge_attr is not None
        batch = self.gps_layer(batch)
        if isinstance(batch, tuple):  # 说明你传的是 (x, None)
            x = batch[0]              # 提取节点特征
        else:
            x = batch.x    
        atom_x1= atom_in + F.relu(x)
        atom_x = F.dropout(atom_x1, self.dropout, training=self.training)
        # print('len(dims) - 2')
        return atom_x
...
class Protein_PNAConv(nn.Module):
    ...
    def forward(self, x, prot_edge_index, prot_edge_attr):
        ...
        deg = pyg.utils.degree(prot_edge_index[1], num_nodes=x_in.size(0))
        ...
        # print(f"prot_edge_index size: {prot_edge_index.size()}")
        # print(f"prot_edge_attr size: {prot_edge_attr.size()}")
        # print(f"deg size: {deg.size()}")
        # print(f"log_deg size: {log_deg.size()}")


        batch = Batch(x=x_in, edge_index=prot_edge_index, edge_attr=prot_edge_attr, deg=deg, log_deg=log_deg)
        assert batch.edge_attr is not None
        batch = self.gps_layer(batch)
        if isinstance(batch, tuple):  # 说明你传的是 (x, None)
            x = batch[0]              # 提取节点特征
            ...
            x = batch.x    
        new_x1= x_in + F.relu(x)
        
        x = F.dropout(new_x1, self.dropout, training=self.training)
...
class DrugProteinConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ## KAN
        self.clique_mlp = KAN_linear(atom_channels*2, atom_channels)
        self.residue_mlp = KAN_linear(residue_channels*2, residue_channels)
