...
class Drug_PNAConv(nn.Module):
    ...
    def forward(self, atom_x, bond_x, atom_edge_index):
        ...
        deg = pyg.degree(atom_edge_index[1], num_nodes=atom_in.size(0))
        log_deg = torch.log(deg + 1).unsqueeze(-1)

        # PNA 
        h_local = self.atom_conv(atom_x, atom_edge_index, bond_x)
        h_local = self.atom_norm(h_local)
        ...
        # gate
        gate_value = self.gate(h_local + h_attn)  # 
        atom_x1 = gate_value * h_local + (1 - gate_value) * h_attn  # 
...
class Protein_PNAConv(nn.Module):
    ...
    def forward(self, x, prot_edge_index, prot_edge_attr):
        ...
        # 
        deg = pyg.degree(prot_edge_index[1], num_nodes=x_in.size(0))
        log_deg = torch.log(deg + 1).unsqueeze(-1)

        # PNA 
        h_local = self.conv(x, prot_edge_index, prot_edge_attr)
        h_local = self.norm(h_local)
        ...
        # 
        gate_value = self.gate(h_local + h_attn)  #
        new_x1 = gate_value * h_local + (1 - gate_value) * h_attn  # 
...
class DrugProteinConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ## KAN
        self.clique_mlp = KAN([atom_channels*2, atom_channels*2, atom_channels])
        self.residue_mlp = KAN([residue_channels*2, residue_channels*2, residue_channels])
        ## temperature
        self.t = t
        # self.logit_scale = nn.Parameter(torch.ones([])) # * np.log(1 / 0.07))
